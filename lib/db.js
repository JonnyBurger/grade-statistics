...
exports.reset = function () {
	...
	if (
		process.env.PGHOST.includes('46.101.96.149') &&
		process.env.PGDATABASE.includes('bestande')
	) {
		throw new Error('Do not reset prod db!');
	}
...
exports.getAllStats = function (module, institution) {
	return co(function * () {
		...
		const detailed = _.sortBy(detailsCombined, d =>
			uzhSemesters.all.indexOf(d.semester)
		).reverse();
		const total = exports.reducePredefined(
...
exports.reducePredefined = function (predefined) {
	...
	return {
		...
		failed,
		count,
		average: count === 0 ? Number.NaN : Math.round((grades / count) * 100) / 100
	};
};
