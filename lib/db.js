...
exports.reset = function () {
	...
	if (process.env.PGHOST.indexOf('46.101.96.149') !== -1 && process.env.PGDATABASE.indexOf('bestande') !== -1) {
		throw new Error('Do not reset prod db!');
	}
...
exports.insertStat = function (student, module, grade, semester, repeated, institution) {
	...
	return new Promise((resolve, reject) => {
		...
		`, [student, module, grade, semester, repeated, institution], function (err, result) {
			if (err) {
				reject(err);
			} else {
				resolve(result);
...
exports.studentIsOptedIn = function (student, institution) {
	return new Promise((resolve, reject) => {
		connection.query(`
			select cast(count(*) as integer) from grades
			where student = $1::text
			and institution = $2::text
		`, [student, institution], function (err, result) {
			if (err) {
				reject(err);
			} else {
...
exports.getTotalStats = function (module, institution) {
	return new Promise((resolve, reject) => {
		...
		`, [String(module), institution], function (err, result) {
			if (err) {
				reject(err);
			} else {
				resolve(result);
	...
	return new Promise((resolve, reject) => {
		var query = dataset.points.map(point => {
			return `
				...
				values ('${dataset.source}', ${point.module}, ${point.average}, '${point.semester}', ${point.passed}, ${point.failed}, '${dataset.institution}');
			`;
		}).join('\n');
	...
	return new Promise((resolve, reject) => {
		...
		connection.query(`
			select width_bucket(grade, 1, 6, ${buckets}) as bucket, count(*) as count
				...
				group by bucket
				order by bucket
		`, [String(module), institution], (err, result) => {
			if (err) {
				reject(err);
			} else {
...
exports.addOptOut = function (student, institution) {
	return new Promise((resolve, reject) => {
		...
		`, [student, new Date(), institution], function (err, result) {
			if (err) {
				reject(err);
			} else {
				resolve(result);
